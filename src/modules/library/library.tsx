...
const Library = () => {
    ...
    // Options derived from materials seen so far (accumulated across fetches)
    ...
    const mergeOptions = (prev: Option[], incoming: Option[]) => {
        const map = new Map(prev.map((o) => [o.value, o.label]));
        incoming.forEach((o) => {
            if (o.value && !map.has(o.value)) map.set(o.value, o.label);
        });
    ...
    const fetchData = async () => {
        try {
            ...
            const res = await getMaterials({
                ...(subjectId !== "ALL" && { subjectIds: [subjectId] }),
                ...(educationLevelId !== "ALL" && { educationLevelIds: [educationLevelId] }),
                skip: (page - 1) * PAGE_LIMIT,
                take: PAGE_LIMIT,
                search_column: ["name"],
                search,
            ...
            // Derive filter dropdown options from the materials we've seen
            const subjOpts = list
    ...
    useEffect(() => {
        const timer = setTimeout(fetchData, 300);
        return () => clearTimeout(timer);
    }, [subjectId, educationLevelId, page, search]);

    const subjectSelectOptions = useMemo(
        () => [{ label: "All Subjects", value: "ALL" }, ...subjectOptions],
        [subjectOptions]
    );
    const levelSelectOptions = useMemo(
        () => [{ label: "All Levels", value: "ALL" }, ...levelOptions],
        [levelOptions]
    );
